variable "app_name" {
  ...
  type        = string
  default     = "nodejs-fastify-boilerplate"
}
...
variable "app_image" {
  description = <<-EOT
    Full ECR image URI for the Fastify app.
    ...
    Example: 123456789.dkr.ecr.us-east-1.amazonaws.com/nodejs-fastify-boilerplate/app:latest
  EOT
  type        = string
}
...
variable "worker_image" {
  description = <<-EOT
    ...
    Example: 123456789.dkr.ecr.us-east-1.amazonaws.com/nodejs-fastify-boilerplate/worker:latest
  EOT
  type        = string
}
