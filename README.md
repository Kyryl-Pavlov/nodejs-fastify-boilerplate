<div align="center">
  <img src="docs/favicon.png" width="80" height="80" alt="Node.js Fastify Boilerplate"/>
</div>
...
Ships fully wired: **JWT authentication** (`@fastify/jwt`), **PostgreSQL** with Prisma ORM and migrations, **AWS S3** file uploads with presigned URLs, async event processing via **AWS SQS + Lambda**, **Redis** caching, **Nginx** reverse proxy with DDoS protection, structured logging to **Sentry / CloudWatch / Loki**, **Prometheus** metrics with **Grafana** dashboards, **Docker Compose** full-stack setup, and a **Vitest** test suite (unit · integration · E2E) — all production-wired from the first commit.
...
- REST API (Fastify plugins, versioned at `/api/v1/`)
- GraphQL API (Mercurius, SDL-first, at `/graphql`)
- JWT authentication (access + refresh tokens, via `@fastify/jwt`)
...
GraphiQL is available at http://localhost/graphql. The same flow works over GraphQL using mutations and queries.

> For authenticated requests in GraphiQL, add an HTTP header: `{"Authorization": "Bearer <access_token>"}`
...
> GraphQL field names use **camelCase** (`accessToken`, `mediaId`, `expiresIn`) even though REST uses snake_case — the SDL is hand-written camelCase directly, since (unlike Strawberry for Python) Mercurius doesn't auto-convert field casing.
...
All external traffic enters through **Nginx on port 80** (`nginx/nginx.conf`). The Fastify app is not exposed publicly — Nginx proxies to it internally at `app:5000`.
...
If `REDIS_URL` is not set, `fastify.cache` is `null` and all cache calls are skipped — the app degrades gracefully without errors.
...
**File uploads are restricted** — only `jpg`, `jpeg`, `png`, `gif`, `webp`, `pdf`, `mp4`, `mov` are accepted. Anything else returns `415`. Maximum upload size is 50 MB (Fastify's `bodyLimit`). To extend the allowlist, edit `ALLOWED_EXTENSIONS` in [services/app/src/routes/v1/media.ts](services/app/src/routes/v1/media.ts).
...
**JWT algorithm is pinned** — `@fastify/jwt`'s default HS256, with an explicit `type: "access" | "refresh"` claim checked on every verify, preventing a refresh token from being used where an access token is required (and vice versa).
...
**Content-Security-Policy is not set** — a CSP covering GraphiQL requires tuning per deployment. Add it once you know your frontend's allowed origins.
...
| Non-root containers                   | Fastify app runs as `app` system user; Lambda runs as `nobody` — no root access if a container is compromised                          |
| ECS task role scoped to send-only SQS | Fastify app can only `SendMessage`. Only the Lambda worker can `ReceiveMessage` and `DeleteMessage`                                    |
...
#    docker build -f services/app/Dockerfile -t <registry>/nodejs-fastify-boilerplate-dev/app:latest .
#    docker push <registry>/nodejs-fastify-boilerplate-dev/app:latest
...
The `app` service runs Fastify under `tsx watch` with Node's built-in inspector (port 9229) and mounts the source directory as a volume, so code changes are reflected immediately without rebuilding.
...
The Fastify app exposes a Prometheus-format metrics endpoint at `http://localhost:5000/metrics`, via `prom-client`. Prometheus scrapes it every 15 seconds. Query directly at `http://localhost:9090`.
...
```logql
{app="nodejs-fastify-boilerplate"}                    # all logs
{app="nodejs-fastify-boilerplate", level="error"}     # errors only
{app="nodejs-fastify-boilerplate"} |= "upload"        # text search
```
